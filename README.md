...
int main() {
    ...
    for(Eigen::Index i = 0; i < result.eigVal().size(); ++i) {
        std::println("lambda[{}] = {:.16e}", i, result.eigVal()(i));
    }
}
```
...
int main() {
    ...
    auto result = solver.get_result();
    std::println("generalized lambda = {:.16e}", result.eigVal()(0));
    std::println("recovered A eigenvalue = {:.16e}", 1.0 / result.eigVal()(0));
}
```
