...
namespace grit {
    namespace internal {
        template<typename Variant, typename Fn> decltype(auto) visit_solver_source(const Variant &variant, Fn &&fn) {
            return std::visit(
                [&](auto *src) -> decltype(auto) {
                    if(src == nullptr) throw std::logic_error("ResultView has no solver source");
                    return fn(*src);
                },
                variant);
        }
    ...
    namespace form {
        template<typename Scalar_, grit::Form form_> ResultView<Scalar_> base<Scalar_, form_>::get_result_view() const { return ResultView<Scalar_>(*this); }

        template<typename Scalar_, grit::Form form_> Result<Scalar_> base<Scalar_, form_>::get_result() const { return get_result_view().to_result(); }
    }

    template<typename Scalar_> template<grit::Form form> ResultView<Scalar_>::ResultView(const form::base<Scalar, form> &source_) : source(&source_) {}

    template<typename Scalar_> const typename ResultView<Scalar_>::VectorReal &ResultView<Scalar_>::eigVal() const {
        return internal::visit_solver_source(source, [](const auto &src) -> const VectorReal & { return src.status.eigVal; });
    }

    template<typename Scalar_> typename ResultView<Scalar_>::MatrixView ResultView<Scalar_>::eigVecs() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::VectorView ResultView<Scalar_>::rNormsAbs() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_operator_inner() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_matvecs() const {
        return internal::visit_solver_source(source, [](const auto &src) { return static_cast<RealScalar>(src.status.time_matvecs.get_time()); });
    }

    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_matvecs_inner() const {
        return time_matvecs_a_inner() + time_matvecs_b_inner();
    }

    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_matvecs_a() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_matvecs_b() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_matvecs_a_inner() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_matvecs_b_inner() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_preconditioner_inner() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_preconditioner_update() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_preconditioner_update_inner() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_preconditioner_update_total() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_preconditioner_apply_inner() const {
        return internal::visit_solver_source(source,
                                             [](const auto &src) { return static_cast<RealScalar>(src.status.time_preconditioner_apply_inner.get_time()); });
    }

    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_preconditioner_apply_total() const {
        return internal::visit_solver_source(source, [](const auto &src) {
            return static_cast<RealScalar>(src.status.time_precond.get_time() + src.status.time_preconditioner_apply_inner.get_time());
        });
    }

    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_project_left_inner() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_project_right_inner() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_residual_correction() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_build() const {
        return internal::visit_solver_source(source, [](const auto &src) { return static_cast<RealScalar>(src.status.time_build.get_time()); });
    }

    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_status_update() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_orthogonalize() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_orthonormalize() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_orth_project() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_orth_factor() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_orth_update() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_orth_refresh() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_orth_mask() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_diagonalize() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_extract_ritz() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::time_restart() const {
        return internal::visit_solver_source(source, [](const auto &src) { return static_cast<RealScalar>(src.status.time_restart.get_time()); });
    }

    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::inner_error_last() const {
        return internal::visit_solver_source(source, [](const auto &src) { return src.status.inner_error_last; });
    }

    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::inner_tol_last() const {
    ...
    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::op_norm_estimate() const {
        return internal::visit_solver_source(source, [](const auto &src) { return src.status.op_norm_estimate; });
    }

    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::condition_a() const {
        return internal::visit_solver_source(source, [](const auto &src) { return src.status.condition_a; });
    }

    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::condition_b() const {
        return internal::visit_solver_source(source, [](const auto &src) { return src.status.condition_b; });
    }

    template<typename Scalar_> typename ResultView<Scalar_>::RealScalar ResultView<Scalar_>::gap() const {
        return internal::visit_solver_source(source, [](const auto &src) { return src.status.gap; });
    }

    template<typename Scalar_> bool ResultView<Scalar_>::residual_converged() const {
        return internal::visit_solver_source(source, [](const auto &src) { return src.status.residual_converged; });
    }

    template<typename Scalar_> bool ResultView<Scalar_>::residual_below_gap() const {
        return internal::visit_solver_source(source, [](const auto &src) { return src.status.residual_below_gap; });
    }

    template<typename Scalar_> std::string_view ResultView<Scalar_>::residual_correction_active_name() const {
        return internal::visit_solver_source(source, [](const auto &src) { return enum2sv(src.auto_residual_correction.active); });
    }

    template<typename Scalar_> std::string_view ResultView<Scalar_>::residual_correction_iteration_name() const {
        return internal::visit_solver_source(source, [](const auto &src) { return enum2sv(src.auto_residual_correction.iteration_method); });
    }

    template<typename Scalar_> Eigen::Index ResultView<Scalar_>::auto_cheap_olsen_iters() const {
        return internal::visit_solver_source(source, [](const auto &src) { return src.auto_residual_correction.cheap_olsen_iters; });
    }

    template<typename Scalar_> Eigen::Index ResultView<Scalar_>::auto_jd_outer_iters_since_probe() const {
        return internal::visit_solver_source(
            source, [](const auto &src) { return src.get_num_consecutive_correction_samples(grit::ResidualCorrectionType::JACOBI_DAVIDSON); });
    }

    template<typename Scalar_> const std::vector<Eigen::Index> &ResultView<Scalar_>::cheap_olsen_to_jd_switch_outer_iters() const {
        return internal::visit_solver_source(
    ...
    template<typename Scalar_> const std::vector<Eigen::Index> &ResultView<Scalar_>::jd_to_cheap_olsen_switch_outer_iters() const {
        return internal::visit_solver_source(
