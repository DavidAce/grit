...
namespace bench_generalized {
    ...
    std::vector<std::string> normalized_cli_args(int argc, char **argv) {
        ...
        for(auto i = argc - 1; i > 0; --i) {
            ...
            if(arg == "--use-b-inner-product") arg = "--use-b-inner-product=true";
            if(arg == "--use-jd-b-only") arg = "--use-jd-b-only=true";
    ...
    void configure_cli(CLI::App &app, CliOptions &opts) {
        ...
        app.add_option("--matrix-a-path", opts.matrix_a_path, "Path to the generalized A Matrix Market .mtx file")->required()->check(CLI::ExistingFile);
        app.add_option("--matrix-b-path", opts.matrix_b_path, "Path to the generalized B Matrix Market .mtx file")->required()->check(CLI::ExistingFile);
        app.add_option("--initial-guess", opts.initial_guess, "Path to HDF5 file with /grit/generalized/<algo>/eigvecs initial guess (legacy /grit/generalized/eigvecs also accepted)")->check(CLI::ExistingFile);
        ...
        app.add_option("--reltol", opts.reltol, "Relative-to-initial absolute residual-norm convergence tolerance");
        ...
        auto *auto_sat_rnorm_threshold_opt = app.add_option("--auto-sat-rnorm-threshold", opts.auto_sat_rnorm_threshold, "AUTO rescaled-residual saturation tolerance");
        auto *auto_jd_start_rnorm_threshold_opt = app.add_option("--auto-jd-start-rnorm-threshold", opts.auto_jd_start_rnorm_threshold, "rescaled residual norm below which AUTO may activate JD; 0 disables it");
        ...
        auto *auto_cheap_probe_factor_opt = app.add_option("--auto-cheap-probe-factor", opts.auto_cheap_probe_factor,
                       "Cheap probe must improve the Ritz value by this factor times max(absolute rnorm_abs^2, roundoff scale)");
        ...
        app.add_option("--use-b-inner-product", opts.use_b_inner_product, "Use B-metric inner products, or use [false,true]")->delimiter(',');
        ...
        auto *use_jd_b_only_opt = app.add_option("--use-jd-b-only", opts.use_jd_b_only, "Use only B in generalized Jacobi-Davidson correction, or use [false,true]")->delimiter(',');
    ...
    void normalize_options(CliOptions &opts) {
        ...
        if(opts.use_b_inner_product.empty()) throw std::runtime_error("--use-b-inner-product must not be empty");
    ...
    std::vector<Options> expand_sweep(const CliOptions &cli, Eigen::Index matrix_rows) {
        ...
        const auto &b_inner_product_values          = cli.use_b_inner_product;
        const auto &jd_b_only_values                = cli.use_jd_b_only;
                            ...
                            for(auto ritz : ritz_values)
                                for(auto residual_correction : residual_correction_values)
                                    for(auto use_refined_rayleigh_ritz : refined_rayleigh_ritz_values)
                                        for(auto use_b_inner_product : b_inner_product_values)
                                            for(auto use_jd_b_only : jd_b_only_values)
                                                for(auto use_adaptive_inner_tolerance : adaptive_inner_tolerance_values) {
                                            Options opts;
                                            opts.case_id       = case_id++;
                                            opts.algo          = cli.algo;
                                            opts.matrix_a_path = cli.matrix_a_path;
                                            opts.matrix_b_path = cli.matrix_b_path;
                                            ...
                                            opts.use_b_inner_product          = use_b_inner_product;
                                            opts.use_jd_b_only                = use_jd_b_only;
