...
namespace bench_standard {
    namespace {
        using GdSolver      = grit::standard::gdplusk<Scalar>;
        using LanczosSolver = grit::standard::lanczos<Scalar>;
        using LobpcgSolver  = grit::standard::lobpcg<Scalar>;
        ...
        GdpluskSnapshot make_solver_snapshot(const Options &opts, int rep, unsigned int rep_seed, const GdSolver &solver, grit::ResultView<Scalar> view) {
            ...
            snapshot.jd_to_cheap_olsen_switch_outer_iters = to_i64_vector(view.jd_to_cheap_olsen_switch_outer_iters());
            return snapshot;
        }
    ...
    SolveResult solve_once(const SparseMatrix &matrix, Options opts, int rep) {
        switch(opts.algo) {
            case Algo::gdplusk: return solve_once_impl<GdSolver>(matrix, opts, rep);
            case Algo::lanczos: return solve_once_impl<LanczosSolver>(matrix, opts, rep);
            case Algo::lobpcg: return solve_once_impl<LobpcgSolver>(matrix, opts, rep);
        }
