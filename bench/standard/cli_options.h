...
namespace bench_standard {
    struct CliOptions {
        ...
        std::vector<double> tol                           = {1e-12}; /*!< Absolute residual-norm convergence tolerance, optionally as a sweep list. */
        double              tol_rnorm_relative            = 0.0;     /*!< Relative residual-norm convergence tolerance; zero disables it. */
        double              sat_eigval_threshold          = 0.0;     /*!< Eigenvalue saturation threshold for stopping; zero disables this stop. */
        double              sat_rnorm_threshold           = 0.0;     /*!< Residual-norm saturation threshold for stopping; zero disables this stop. */
        ...
        double              auto_sat_rnorm_threshold      = 1e-2;    /*!< Residual-norm saturation threshold for AUTO JD activation. */
        double              auto_jd_start_rnorm_threshold = 1e-5;    /*!< Residual norm below which AUTO may activate JD; zero disables it. */
        int                 auto_cheap_probe_interval     = 5;       /*!< JD steps before AUTO forces a cheap-Olsen probe. */
        double              auto_cheap_probe_factor   = 1.0; /*!< Cheap probe must improve the Ritz value by this factor times max(rnorm^2, roundoff scale). */
        unsigned int        seed                      = 0;
        std::string         ritz                      = "SR";
        spdlog::level::level_enum log_level           = spdlog::level::warn;
        std::string               residual_correction = "none";
        std::vector<bool>         refined_rayleigh_ritz        = {false};
